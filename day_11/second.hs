...
painter :: Int -> Tape -> (CoorSet, [Coor])
painter start t = painted
  where
    o = reverse . outs . execute $ Computer{ptr=0, tape=t, ins=(start:i), outs=[], relBase=0}
    (i, painted) = process o '^' (0, 0) Set.empty []

visualize :: CoorSet -> [String]
visualize cs = [[if (x, y) `Set.member` cs then '@' else ' '| y <- [mny..mxy]] | x <- [mnx..mxx]]
  where
    ls =  Set.toList cs
    mnx = minimum [x | (x, _) <- ls]
    mxx = maximum [x | (x, _) <- ls]
    mny = minimum [y | (_, y) <- ls]
    mxy = maximum [y | (_, y) <- ls]

main :: IO()
main = do
  contents <- readFile "input"
  let tape = getTape contents
  let (white, _) = painter 1 tape
  mapM_ print $ visualize white

...
getThrust (sa:sb:sc:sd:se:_) tape = last e
  where
    a = run tape (sa:0:e)
    b = run tape (sb:a)
    c = run tape (sc:b)
    d = run tape (sd:c)
    e = run tape (se:d)
    run t i = reverse . outs $ execute Computer {ptr=0, tape=t, ins=i, outs=[]}

getMaxThrust :: Tape -> Int
getMaxThrust tape = maximum [getThrust s tape | s <- permutations [5..9]]
